                ...
                if (i == queLength)
                {
                    ...
                    if (Physics.Raycast(ray, out hit, rayRange))
                    {
                        if (hit.transform.tag == "Ground")
                        {
                            ...
                            if ((hitXZ.x != transform.position.x) || (hitXZ.z != transform.position.z))
                            {
                                positionQue[i] = hitXZ;
    ...
    IEnumerator StartLerpSequence(Vector3[] toLerpArray, float totalTime)
    {
        ...
        while (timeRatio < 1)
        {
            ...
            { // Clean up here
                ...
                try { transform.GetComponent<Rigidbody>().velocity = Vector3.zero; }
                catch { Debug.Log("Player has no rigidbody"); }
                if (i + 1 < queLength)
                {
                    ++i;
                    startTime = Time.time;
